...
mat construct_ultra_cost_mat(const mat& ux, const mat& uy, const mat& coupling, double& p){
  int n = ux.n_cols;
  int m = uy.n_cols;
  mat cost_mat;
  cost_mat.zeros(n,m);
    ...
    for (size_t j=0; j<m; j++){
        ...
        for (size_t k=0; k<n; k++){
            for (size_t l=0; l<m; l++){
                tmp=tmp+pow(delta_infinity(ux.at(i,k),uy.at(j,l)),p)*coupling.at(k,l);
...
// Hier endet der essentielle Code. Der Rest sind zusammen geschusterte Wrapper-Funktionen.
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  mat cost_mat(1,1);  
  cost_mat = construct_ultra_cost_mat(ux, uy, coupling, p);
  //timestamp();
